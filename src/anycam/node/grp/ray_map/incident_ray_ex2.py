...
def Create(*, bForce=False):
    ...
    sGrpName = "AnyCam.Refractor.IndicentRayDataEx2"
    ...
    if bUpdate is True:
        ...
        lInputs = [
            ...
            [sPixSize_bu, "NodeSocketFloat", 0.0],
            [sPixCntX, "NodeSocketFloat", 0.0],
            [sPixCntY, "NodeSocketFloat", 0.0],
        ]
        ...
        skOrigin = nsh.vector.CombineXYZ(
            ngMain, "Origin", nodIn.outputs[sOrigX_bu], nodIn.outputs[sOrigY_bu], 0.0
        )
        ...
        skNormal = nsh.vector.TransformWorldToObject(
            ngMain, "Normal (local)", lGeoInfo["Normal"]
        )
        nalign.Relative(lGeoInfo, (1, 1), skNormal, (0, 0), tNodeSpace)

        skIncoming = nsh.vector.TransformWorldToObject(
            ngMain, "Incoming (local)", lGeoInfo["Incoming"]
        )
        ...
        skPntObj = nsh.vector.TransformPointWorldToObject(
            ngMain, "Point on Object", lGeoInfo["Position"]
        )
        nalign.Relative(lGeoInfo, (1, 0), skPntObj, (0, 0), tNodeSpace)

        skRelPnt = nsh.vector.Subtract(
            ngMain, "Point rel. to Origin", skPntObj, skOrigin
        )
        ...
        # Evaluate tangent
        skProjIncNorm = nsh.vector.Project(
            ngMain, "Project Inc. on Norm.", skIncoming, skNormal
        )
        nalign.Relative(skIncoming, (0, 1), skProjIncNorm, (0, 0), tNodeSpace)

        skProjIncPlane = nsh.vector.Subtract(
            ngMain, "Project Inc. on Plane", skIncoming, skProjIncNorm
        )
        ...
        skImgSizeY_bu = nsh.math.Multiply(
            ngMain,
            "Img Size Y (bu)",
            nodIn.outputs[sPixCntY],
            nodIn.outputs[sPixSize_bu],
        )
        nalign.Relative(skOrigin, (0, 0), skImgSizeY_bu, (0, 1), tNodeSpace)

        skImgSizeX_bu = nsh.math.Multiply(
            ngMain,
            "Img Size X (bu)",
            nodIn.outputs[sPixCntX],
            nodIn.outputs[sPixSize_bu],
        )
        nalign.Relative(skImgSizeY_bu, (0, 0), skImgSizeX_bu, (0, 1), tNodeSpaceSmall)

        skSepPnt = nsh.vector.SeparateXYZ(ngMain, "Point on Object", skPntObj)
        nalign.Relative(skRelPnt, (0, 0), skSepPnt, (0, 1), tNodeSpace)

        skRelV = nsh.math.Divide(ngMain, "Rel. V", skSepPnt["Y"], skImgSizeY_bu)
        nalign.Relative(skSepPnt, (1, 0), skRelV, (0, 1), tNodeSpace)

        skRelU = nsh.math.Divide(ngMain, "Rel. U", skSepPnt["X"], skImgSizeX_bu)
        nalign.Relative(skRelV, (0, 0), skRelU, (0, 1), tNodeSpaceSmall)

        skRelUV = nsh.vector.CombineXYZ(ngMain, "Rel UV", skRelU, skRelV, 0.0)
        nalign.Relative(skRelV, (1, 0), skRelUV, (0, 0), tNodeSpace)

        skUVCoord = nsh.vector.Add(ngMain, "UV Coord", skRelUV, (0.5, 0.5, 0.0))
        nalign.Relative(skRelUV, (1, 0), skUVCoord, (0, 0), tNodeSpaceSmall)

        ngMain.links.new(skUVCoord, nodOut.inputs[sUVCoord])
